...
function pruneInteractionsToKnownPosts(posts: Pick<AppPost, "id">[], interactions: InteractionsMap) {
...
export default function Page() {
  ...
  const syncSharedState = async ({
    ...
    const safePosts = nextPosts ? serializePostsForStorage(nextPosts) : undefined;
    const safeInteractions = nextInteractions
      ? pruneInteractionsToKnownPosts(nextPosts ?? postsRef.current, nextInteractions)
      : undefined;
    const headers = await getAuthenticatedHeaders({
      "Content-Type": "application/json",
    });
    void fetch("/api/state", {
      ...
      headers,
      body: JSON.stringify({
        ...(safePosts ? { posts: safePosts } : {}),
        ...(safeInteractions ? { interactions: safeInteractions } : {}),
  ...
  useEffect(() => {
    if (!hasLoadedDataRef.current || typeof window === "undefined") return;
    window.localStorage.setItem(INTERACTIONS_KEY, JSON.stringify(pruneInteractionsToKnownPosts(posts, interactions)));
  }, [interactions, posts]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("crumbz-native-auth", onNativeAuth);
  }, []);
                      ...
                      <Button
                        ...
                        onPress={async () => {
                          ...
                          try {
                            ...
                            const { data, error: oauthErr } = await supabaseBrowser.auth.signInWithOAuth({
                              provider: "google",
                              options: {
                                redirectTo: "crumbz://login-callback",
      ...
      <div className="mx-auto min-h-screen w-full max-w-md bg-white px-4 pb-40 font-[family-name:var(--font-manrope)]" style={{ paddingTop: isNativePlatform ? "env(safe-area-inset-top)" : "1.25rem" }}>
                  ...
                  <Card className="rounded-[28px] border border-[#FFE1B3] bg-white shadow-[0_18px_50px_rgba(254,138,1,0.08)]">
                    <CardBody className="gap-5 p-5">
                      <div>
                        ...
                        <p className="mt-2 text-sm leading-6 text-[#6c7289]">
                          this cannot be undone. if you come back later with Google or email, crumbz will treat you like a
        ...
        {selectedStoryPost || notificationsOpen || selectedOwnArchiveOpen || selectedOwnPost || selectedProfileEmail || settingsModalOpen ? null : (
          <BottomNav
            ...
            tabLabels={{
              feed: copy.tabs.feed,
              favorites: copy.tabs.favorites,
              rewards: copy.tabs.rewards,
